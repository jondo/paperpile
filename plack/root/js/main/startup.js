...
// Stage 0 
//
// Check if server is already running, if not start the plack
  ...
  Paperpile.Ajax({
    ...
    failure: function(response) {
      ...
      if (IS_QT) {
        ...
        // Start plack server after the first failed ping attempt
        if (Paperpile.pingAttempts == 1) {

          //Set up signals/slot connection for plack process
          QRuntime.plackReady.connect(function() {
            QRuntime.log("Plack succesfully started.");
            Paperpile.stage0();
          });

          QRuntime.plackExit.connect(
            function(error) {
          ...
          QRuntime.plackRead.connect(function(string) {
