...
has plack_dir  => ( is => 'rw' );    # plack directory
...
sub make_dist {
  ...
  my ( $dist_dir, $plack_dir, $qt_dir ) = ( $self->dist_dir, $self->plack_dir, $self->qt_dir );
  ...
  $self->echo("Copying Plack files.");
  $sub_dir = 'osx/Contents/Resources' if ($platform eq 'osx');

  mkpath( catfile("$dist_dir/$sub_dir/plack") );
  $list = $self->_get_list( $plack_dir, \@ignore );
  $self->_copy_list( $list, $plack_dir, "$sub_dir/plack" );

  # Update configuration file for current build
  my $yaml   = "$dist_dir/$sub_dir/plack/conf/settings.yaml";
  my $config = LoadFile($yaml);
...
sub minify {
  ...
  my $data = LoadFile("$plack_dir/conf/resources.yaml");

  my $all_css = "$plack_dir/root/css/all.css";

  unlink($all_css);

  foreach my $file ( @{ $data->{css} } ) {
    `cat $plack_dir/root/$file >> $all_css`;
  }

  my $all_js = "$plack_dir/root/js/all.js";

  unlink($all_js);

  foreach my $file ( @{ $data->{js} } ) {
    `cat $plack_dir/root/$file >> tmp.js`;
  }
  my @plugins = glob("$plack_dir/root/js/??port/plugins/*js");
