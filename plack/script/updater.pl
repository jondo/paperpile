...
my $app_settings = YAML::LoadFile("$plack_dir/conf/settings.yaml")->{app_settings}
  || die($!);
...
my $needs_sudo = ( -w "$plack_dir/conf/settings.yaml" ) ? 0 : 1;
...
if ($needs_sudo) {
  ...
  # Get plack base dir via the include path of the perl binary
  my $plack_path = $INC[0];
  $plack_path =~ s!/perl5/$platform/base!!;

  my $call = "$plack_path/perl5/$platform/bin/paperperl $plack_path/script/updater.pl --update";
