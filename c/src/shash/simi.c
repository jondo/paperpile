 ...
 *
 * Public domain
 * 2007,2009 Viliam Holub <holub@dsrg.mff.cuni.cz>
 */
 ...
 *
 * A slightly modified algorithm I found in some AMD documentation.
 */
int
...
charikar_hash64( const char *data, long data_length)
{
	...
	while ((get_next_token( &data, data_end, &token_start, &token_length))) {
	  ...
	  /* ONLY CONSIDER WORDS WITH MORE THAN THREE CHARS */
	  if ( token_length > 3 ) {
