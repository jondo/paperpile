...
static GString* getInfoString(Dict *infoDict, char *key, UnicodeMap *uMap);
...
int main(int argc, char *argv[]) {
  ...
  if ((uMap = globalParams->getTextEncoding())) {
    ...
    if (info.isDict()) {
      ...
      // Don't use function getInfoString, it gave weird results when
      // decoding unicode and I don't understand what's going on so we
      // just pass the title without decoding which is at least better
      // than not title at all
 ...
 if (!doc->okToCopy())
  ...
  // Write xml file
  xmlOut = new XmlOutputDev(textFileName, fileName, title, physLayout, verbose, nsURI, cmd);
...
static GString* getInfoString(Dict *infoDict, char *key, UnicodeMap *uMap) {
  ...
  if (infoDict->lookup(key, &obj)->isString()) {
    s1 = obj.getString();
    printf("-->%s", obj.getString()->getCString());
	...
	++i;
      ...
      printf("-->%s", buf);
      string string1(buf, 8);
      fix_utf8_string(string1);
