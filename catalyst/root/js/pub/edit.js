Paperpile.MetaPanel = Ext.extend(Ext.form.FormPanel, {
    ...
    initForm: function(pubType) {
        ...
        var list = ['ARTICLE', 'BOOK', 'INCOLLECTION', 'INBOOK', 'PROCEEDINGS', 'INPROCEEDINGS', 'MASTERSTHESIS', 'PHDTHESIS', 'MANUAL', 'TECHREPORT', 'UNPUBLISHED', 'MISC'];
        ...
        var cb = new Ext.form.ComboBox({
            ...
            tpl: new Ext.XTemplate('<tpl for="."><div class="x-combo-list-item">{name}<br><span style="color:gray;">{info}</span></div></tpl>'),
            listeners: {
                select: {
    ...
    renderForm: function(pubType) {
        ...
        Ext.each(this.renderMainFields(pubType), function(t) {
            tbodies.push(t);
        });
        ...
        for (var field in Paperpile.main.globalSettings.pub_fields) {
            ...
            if (field === 'journal') {
                ...
                Ext.apply(config, {
                    ...
                    tpl: new Ext.XTemplate('<tpl for="."><div class="x-combo-list-item"><b>{short}</b><br>{long}</div></tpl>'),
                });
            }
