...
Paperpile.Viewport = Ext.extend(Ext.Viewport, {
  ...
  postHook: function(conn, response, options) {
    // options.url is the URL of the request that just completed, thus triggering this hook.
    var url = options.url;
    if (url == Paperpile.Url('/ajax/misc/listen')) {
      this.isListening = false;
    }
    if (!this.hookDelayedTask) {
      this.hookDelayedTask = new Ext.util.DelayedTask(this.executeListenHook, this);
    }
    if (this.isListening) {
      return;
    }
    this.hookDelayedTask.delay(50);
  },
  executeListenHook: function() {
    Paperpile.log("Listening...");
    this.isListening = true;
    Ext.Ajax.request({
      url: Paperpile.Url('/ajax/misc/listen'),
      params: {
        library_version: this.library_version,
      },
      timeout: 1000 * 60 * 60,
      success: function(response) {
        Paperpile.log("  -> Heard something!");
        var json = Ext.util.JSON.decode(response.responseText);
        this.library_version = json.data.library_version;
        this.isListening = false;
	  Paperpile.log(this.library_version);
        (function() {
          Paperpile.main.onUpdate(json.data);
        }).defer(1000);
      },
  ...
  // Callbacks can be specified with either a 'fn', which is the name
  // of a function to be called on the Paperpile.main object,
  // or a statement to be 'eval'ed.
  executeCallback: function(callbackConfig) {
    if (callbackConfig.fn) {
      this[callbackConfig.fn]();
    } else if (callbackConfig.eval) {
      eval(callbackConfig.eval);
    }
  },
