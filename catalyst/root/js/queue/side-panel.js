Paperpile.QueueOverview = Ext.extend(Ext.Panel, {
    ...
    initComponent: function() {
        ...
        this.mainTemplate = new Ext.XTemplate('<div class="pp-box pp-box-side-panel pp-box-style1" style="padding-bottom:15px;">', '  <h2>Tasks</h2>', '  <div id="queue-table"></div>', '</div>', '<div id="queue-button-box" class="pp-box pp-box-side-panel pp-box-style2" style="padding-top:20px;">', '  <center><div id="queue-progress"></div></center>', '  <center><div id="queue-eta" class="pp-queue-eta"></div></center>', '  <center><div id="queue-buttons"></div></center>', '</div>', {
            compiled: true
        });

        this.buttonTemplate = new Ext.XTemplate('<table width="240px;">', '  <tr>', '    <td style="padding:5px;"><div id="pause-button"></div></td>', '    <td style="padding:5px;"><div id="cleanup-button"></div></td>', '    <td style="padding:5px;"><div id="cancel-button"></div></td>', '  </tr>', '</table>', {
            compiled: true
        });

        this.tableTemplate = new Ext.XTemplate('<table class="pp-queue-table">', '  <tr><td></td><td class="pp-queue-table-ok">OK</td><td class="pp-queue-table-error">Failed</td><td class="pp-queue-table-pending">Waiting</td></tr>', '  <tpl for="queue.types">', '    <tr>', '      <tpl if="name==\'PDF_SEARCH\'">', '        <td><span class="pp-queue-type-label-PDF_SEARCH">Search PDF</span></td>', '      </tpl>', '      <tpl if="name==\'PDF_IMPORT\'">', '        <td><span class="pp-queue-type-label-PDF_IMPORT">Import PDF</span></td>', '      </tpl>', '      <td>{num_done}</td><td>{num_error}</td><td>{num_pending}</td>', '    </tr>', '  </tpl>', '</table>', '<p>&nbsp;</p>', '<center>', '<p>Show &nbsp;&nbsp;', '<a href="#" class="pp-textlink" action="show-all-jobs">All</a>&nbsp;|&nbsp;', '<a href="#" class="pp-textlink" action="show-done-jobs">Successful</a>&nbsp;|&nbsp;', '<a href="#" class="pp-textlink" action="show-error-jobs">Failed</a>', '</p>', '</center>', {
            compiled: true
        });

        this.emptyTable = new Ext.XTemplate('<div>', '  <p>No tasks to show.</p>', '  <p><a href="#" class="pp-textlink" action="close-tab">Close Tab</a></p>', '</div>', {
            compiled: true
        });
