...
# prefer a minicpan to the poisoned wireless.
if ( -d 'C:\strawberry\minicpan' ) {
	...
	unless ( $@ ) {
		my $path = URI::file->new('C:\strawberry\minicpan');
	
		if ( LWP::Online::online() ) {
			push @{$Conf->{'conf'}->{'hosts'}}, {
				'path' => $path->path(),
				'scheme' => 'file',
				'host' => ''
			};
		} else {
			unshift @{$Conf->{'conf'}->{'hosts'}}, {
				'path' => $path->path(),
				'scheme' => 'file',
				'host' => ''
			};
...
if ( -d 'C:\minicpan' ) {
	...
	unless ( $@ ) {
		my $path = URI::file->new('C:\minicpan');
...
=cut
       ...
       $Conf->{'conf'}->{'base'} = File::Spec->catdir('C:\strawberry', 'cpanplus');
...
=cut
		### csjewell@cpan.org: This is very slow, so turning off.

        ### this addresses #32248 which requests a possibillity to
        ### turn off custom sources
        $Conf->{'conf'}->{'enable_custom_sources'} = 0;
...
=cut
        ...
        $Conf->{'conf'}->{'source_engine'} = 'CPANPLUS::Internals::Source::SQLite'; 
...
=cut
        ...
        $Conf->{'program'}->{'editor'} = do {
            ...
            can_run('vi')   || can_run('pico') || can_run('notepad')
        };
