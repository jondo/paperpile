...
sub connect {
    ...
    if ( $error_level == 1 ) {
        NetError->throw(
            error => 'Google Scholar blocks queries from this IP.' );
    }
    ...
    my @stats = $tree->findnodes(
        '/html/body/form/table/tr/td[@align="right"]/font[@size="-1"]');
    if ( $stats[0]->as_text() =~
        m/Results\s\d+\s-\s\d+\sof\s(about\s)?([0123456789,]+)\./ )
    {
        my $number = $2;
        $number =~ s/,//g;
...
sub _check_for_better_bibliographic_data {
    ...
    if ( $pub->_www_publisher eq 'ncbi.nlm.nih.gov' ) {
        if ( $pub->linkout() =~
            m/http:\/\/www\.ncbi\.nlm\.nih\.gov\/pubmed\/(\d+)/ )
        {
...
m/http:\/\/www\.pubmedcentral\.nih\.gov\/articlerender\.fcgi\?artid=(\d+)/
    ...
    if ( $pub->_www_publisher eq 'Springer' and $done == 0 ) {
        if ( $pub->linkout() =~
            m/http:\/\/www.springerlink.com\/index\/(\w+)(\.pdf)/ )
        {
            my $tmp_details_link = 'http://www.springerlink.com/content/' . $1;
            my $SpringerLinkPlugin =
              Paperpile::Plugins::Import::SpringerLink->new();
    ...
    if ( $pub->_www_publisher eq 'portal.acm.org' and $done == 0 ) {
        if ( $pub->linkout() =~
            m/http:\/\/portal\.acm\.org\/citation\.cfm\?id=/ )
        {
...
sub complete_details {
    ...
    if ( $pub->_all_versions ne '' and $best_flag == 0 ) {
        ...
        my $response_all_versions =
          $browser->get( $pub->_all_versions . '&num=100' );
        ...
        # Get the BibTeX
        my $full_pub =
          $self->_check_for_better_bibliographic_data( $page->[$best_one] );
        if ($full_pub) {
            return $full_pub;
        }
        else {
...
sub match {
    ...
    if ( $pub->title ) {
        ...
        foreach my $word ( split( /\s+/, $tmp_title ) ) {
            ...
            my @google_stopwords = (
                'about', 'com',  'for', 'from', 'how',  'that',
                'the',   'this', 'was', 'what', 'when', 'where',
                'will',  'with', 'und', 'and',  'www'
            );
            ...
            if ( $pub->title() ) {
                ...
                if ($matchedpub) {
                    ...
                    if ( $page->[0] ) {
                        ...
                        my $doi_response =
                          $browser->get( 'http://dx.doi.org/' . $pub->doi );
                        ( my $doi_content = $doi_response->content ) =~
                          s/({|})//g;
                        $doi_content =~ s/\s+/ /g;
                        $doi_content =~ s/\n//g;
...
sub _find_best_hit {
    ...
    if ( $#google_hits > -1 ) {
        ...
        my @words = ();
        ( my $tmp_orig_title = $orig_pub->title ) =~
          s/(\(|\)|-|\.|,|:|;|\{|\}|\?|!)/ /g;
