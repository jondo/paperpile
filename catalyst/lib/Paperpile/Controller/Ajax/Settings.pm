...
sub pattern_example  {
  ...
  my $paper_root = $c->params->{paper_root};
  my $library_db = $c->params->{library_db};

  my $key_pattern        = $c->params->{key_pattern};
  my $pdf_pattern        = $c->params->{pdf_pattern};
  my $attachment_pattern = $c->params->{attachment_pattern};

  my %data = ();

  foreach my $field ( 'key_pattern', 'pdf_pattern', 'attachment_pattern' ) {
    while ( $c->params->{$field} =~ /\[\s*(.*?)\s*\]/ig ) {
  ...
  my $formatted_key        = $pub->format_pattern( $c->params->{key_pattern} );
...
sub update_patterns  {
  ...
  if ($update_files) {
    $c->model('Library')->rename_files( $c->params->{pdf_pattern},
                                        $c->params->{attachment_pattern});
...
sub set_settings  {
  ...
  # Decode JSON data
  for my $field ( keys %{ $c->params } ) {
    $c->params->{$field} = $json->decode($c->params->{$field});
  }
  ...
  for my $field ( keys %{$c->config->{'user_settings'}}){
    ...
    if ( defined $c->params->{$field} ) {
      $c->model('User')->set_setting( $field, $c->params->{$field} );
    }
  }
  ...
  for my $field ( keys %{$c->config->{'library_settings'}}){
    if ( defined $c->params->{$field} ) {
      $c->model('Library')->set_setting( $field, $c->params->{$field} );
    }
  }
